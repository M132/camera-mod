...
public class CameraSettings {
    ...
    public static final String KEY_VIDEOCAMERA_FPS = "pref_camera_video_fps";
    public static final String KEY_VIDEOCAMERA_RESOLUTION = "pref_video_resolution";
    public static final String KEY_VIDEOCAMERA_VCODEC = "pref_video_vcodec";
    public static final String KEY_VIDEOCAMERA_CONTAINER = "pref_video_container";
    public static final String KEY_VIDEOCAMERA_ACODEC = "pref_video_acodec";
    public static final String KEY_VIDEOCAMERA_ASAMPLERATE = "pref_video_asamplerate";
    public static final String KEY_VIDEOCAMERA_ACHANNELS = "pref_video_achannels";
    public static final String KEY_VIDEOCAMERA_VBITRATE = "pref_video_vbitrate";
    public static final String KEY_VIDEOCAMERA_ABITRATE = "pref_video_abitrate";
    ...
    private void initPreference(PreferenceGroup group) {
        ...
        ListPreference videoResolution = group.findPreference(KEY_VIDEOCAMERA_RESOLUTION);
        ListPreference videoFps = group.findPreference(KEY_VIDEOCAMERA_FPS);
        ...
        // initialize custom preset values
        if (videoResolution != null) {
            List<String> entries = new ArrayList<String>();
            List<Size> sizes = mParameters.getSupportedVideoSizes();
            Size result;

            for (int i=0;i<sizes.size();i++){
                result = (Size) sizes.get(i);
                entries.add(result.width+"x"+result.height);
            }
            CharSequence[] cEntries = entries.toArray(new CharSequence[entries.size()]);

            videoResolution.setEntries(cEntries);
            videoResolution.setEntryValues(cEntries);

            if(Arrays.asList(videoResolution.getEntryValues()).indexOf(videoResolution.getValue()) == -1){
                videoResolution.setValueIndex(0);
            }
        }

        if (videoFps != null) {
            List<int[]> frameRanges = mParameters.getSupportedPreviewFpsRange();
            ...
            for (int i=0;i<frameRanges.size();i++){
                result = frameRanges.get(i);
                entries.add(result[Parameters.PREVIEW_FPS_MIN_INDEX]/1000+" - "+result[Parameters.PREVIEW_FPS_MAX_INDEX]/1000+" FPS");
                entryValues.add(result[Parameters.PREVIEW_FPS_MIN_INDEX]+"-"+result[Parameters.PREVIEW_FPS_MAX_INDEX]);
            }

            for (int i=0;i<frameRates.size();i++){
                result2 = frameRates.get(i);
                entries.add(String.valueOf(result2)+" FPS");
                entryValues.add(result2+"000-"+result2+"000");
            }

            if(entryValues.indexOf("15000-15000") == -1){
                entries.add(mContext.getResources().getString(R.string.pref_force_text)+" 15 FPS");
                entryValues.add("15000-15000");
            }

            if(entryValues.indexOf("30000-30000") == -1){
                entries.add(mContext.getResources().getString(R.string.pref_force_text)+" 30 FPS");
                entryValues.add("30000-30000");
            }

            if(entryValues.indexOf("60000-60000") == -1){
                entries.add(mContext.getResources().getString(R.string.pref_force_text)+" 60 FPS");
                entryValues.add("60000-60000");
            }

            videoFps.setEntries(entries.toArray(new CharSequence[entries.size()]));
            videoFps.setEntryValues(entryValues.toArray(new CharSequence[entryValues.size()]));

            if(Arrays.asList(videoFps.getEntryValues()).indexOf(videoFps.getValue()) == -1){
                videoFps.setValueIndex(0);
            }
        }
    }
